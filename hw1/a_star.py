...
def a_star(puzzle):
    ...
    while len(fringe) > 0:
        # remove the item with the smallest distance+heuristic from the fringe
        ...
        for a in current_state.get_actions():
            ...
            if new_state_str not in distances or new_distance < distances[new_state_str]:
                ...
                heuristic = new_distance + new_state.get_manhattan_distance(goal)
                heapq.heappush(fringe, (heuristic, new_state))
